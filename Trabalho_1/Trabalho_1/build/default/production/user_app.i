...
uint_t __attribute__((reentrant)) round_robin_scheduler(void);
uint_t __attribute__((reentrant)) scheduler(void);
void config_timer0(void);
void __attribute__((picinterrupt(("")))) isr_timer(void);
# 4 "user_app.c" 2
...
# 17 "user_app.c"
pipe_t comunicacao;
sem_t stepper_sem, dc_motor_sem;
byte stepper_buffer, dc_motor_buffer;
...
void config_app_user() {
    ...
    pipe_init(&comunicacao);
    sem_init(&stepper_sem, 0);
    sem_init(&dc_motor_sem, 0);

    __asm("GLOBAL _task_temperature_read, _task_general_control, _task_stepper_motor, _task_dc_motor");
}
...
void task_general_control() {
    ...
    while (1) {
        ...
        if (temperature_level == 'S') {
            LATBbits.LATB6 = 1;
            LATBbits.LATB7 = 1;
            stepper_buffer = '1';
            dc_motor_buffer = 1;
        } else if (temperature_level == 'M') {
            LATBbits.LATB6 = 0;
            LATBbits.LATB7 = 1;
            stepper_buffer = '0';
            dc_motor_buffer = 1;
...
void task_stepper_motor() {
    while (1) {
        ...
        if (stepper_buffer == '1') {
            motor_passo_step(1);
        } else if (stepper_buffer == '0') {
            motor_passo_step(0);
        } else if (stepper_buffer == '2') {
            motor_passo_step(2);
        }
